...
fn http_outbound_request_invalid_version() -> anyhow::Result<()> {
    ...
    cmd!(sh, "node ./src/jco.js run  --jco-dir ./tests/rundir/http_outbound_request_invalid_version --jco-import ./tests/virtualenvs/http.js {wasi_file} hello this '' 'is an argument' 'with 🚩 emoji'").run()?;
    Ok(())
}
