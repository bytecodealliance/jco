...
const TEST_COMPONENTS_PACKAGE_NAME: &str = "extended-jco-test-components";
...
async fn build_test_components(output_dir: impl AsRef<Path>) -> Result<Vec<BuiltComponent>> {
    ...
    cmd.arg(format!("--package={}", TEST_COMPONENTS_PACKAGE_NAME));
    ...
    let targets = meta
        .packages
        .iter()
        .find(|p| p.name == TEST_COMPONENTS_PACKAGE_NAME)
