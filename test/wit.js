...
export async function witTest() {
  suite("WIT", () => {
    ...
    suiteSetup(async function () {
      ...
      witFixturesPath = fileURLToPath(
        new URL("./fixtures/wits", import.meta.url)
      );
    });
    ...
    test("Feature gates (no features)", async () => {
      ...
      deepStrictEqual(imports, ["test:feature-gates/foo"]);
    ...
    test("Invalid WIT produce better error messages", async () => {
      try {
        ...
        fail("test should have errored");
      } catch (err) {
        const errMsg = err.toString();
        ok(errMsg.includes("expected `type`, `resource` or `func`"));
        ok(errMsg.includes("found keyword `static`"));
      }
    });
    ...
    test("different-package sibling WIT files fail w/ good error messages", async () => {
      ...
      try {
        ...
        ok(errMsg.includes("Keep in mind the following rules"));
      }
    });
  });
}
