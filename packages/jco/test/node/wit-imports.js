...
describe("Node API WIT import injection", () => {
    ...
    test.concurrent("is a no-op when run a second time", async () => {
    ...
    test.concurrent("does not alter a user-provided import or add a duplicate dependency", async () => {
    ...
    test.concurrent("recognizes an aliased user-provided import", async () => {
    ...
    test.concurrent("deduplicates repeated detector reports", async () => {
        const root = await fixture({ "app.wit": "package example:app;\nworld app {}\n" });
    ...
    test.concurrent("injects only into an explicitly selected world", async () => {
        const root = await fixture({
            "worlds.wit": [
    ...
    test.concurrent("requires an explicit selection when multiple worlds exist", async () => {
        const source = "package example:app;\nworld first {}\nworld second {}\n";
    ...
    test.concurrent("reports a selected world that cannot be found", async () => {
        const root = await fixture({ "app.wit": "package example:app;\nworld app {}\n" });
    ...
    test.concurrent("ignores world and import text inside comments", async () => {
        const root = await fixture({
            "app.wit": [
                "package example:app;",
    ...
    test.concurrent("supports a WIT file path and preserves CRLF newlines", async () => {
        const root = await fixture({ "app.wit": "package example:app;\r\nworld app {\r\n}\r\n" });
    ...
    test.concurrent("does not inspect or modify a missing WIT path when nothing was detected", async () => {
        expect(await injectNodeWitImports("/does/not/exist", undefined, [])).toBeUndefined();
    });

    test.concurrent("does not overwrite an existing dependency when adding a missing import", async () => {
        const root = await fixture({
            "app.wit": "package example:app;\nworld app {}\n",
    ...
    test.concurrent("leaves the world unchanged when a generated dependency cannot be read", async () => {
        const source = "package example:app;\nworld app {}\n";
        const root = await fixture({ "app.wit": source });
        await expect(
    ...
    test.concurrent("rejects an unterminated selected world without modifying it", async () => {
        const source = "package example:app;\nworld app {\n";
        const root = await fixture({ "app.wit": source });
