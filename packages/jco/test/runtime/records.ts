...
// @ts-ignore
import { instantiate } from "../js-test-components/records/records.js";
// @ts-ignore
...
async function run() {
    ...
    assert.deepEqual(wasm.test.roundtripFlags1({ a: true, b: true }), {
        a: true,
        b: true,
    });

    assert.deepEqual(wasm.test.roundtripFlags2({ c: true }), {
        c: true,
        d: false,
        e: false,
    });
    assert.deepEqual(wasm.test.roundtripFlags2({}), {
        c: false,
        d: false,
        e: false,
    });
    assert.deepEqual(wasm.test.roundtripFlags2({ d: true }), {
        c: false,
        d: true,
        e: false,
    });
    assert.deepEqual(wasm.test.roundtripFlags2({ c: true, e: true }), {
        c: true,
        d: false,
        e: true,
    });
    ...
    {
        const { a, b } = wasm.test.roundtripRecord1({
            a: 0,
            b: { a: true, b: true },
        });
