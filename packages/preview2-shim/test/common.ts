...
export const BASIC_HARNESS_HTML_DIR = fileURLToPath(
    new URL("./fixtures/browser/basic-harness", import.meta.url),
);
 ...
 *
 * @return A Promise that resolves to the created temporary directory
 */
 ...
 * @param args.transpiledOutputDir - Directory that contains a transpiled component to be loaded into the browser
 * @param args.htmlDir - Directory that contains HTML that should be served by the server in all other cases (by default this uses the default harness index.html)
 * @param args.tmpDir - Directory in which to do work (e.g. a dir in /tmp)
 * @param args.outDir - Directory in which to place build outputs (normally inside scratch dir)
 * @param args.debug - Directory in which to place build outputs (normally inside scratch dir)
 */
export async function startTestServer(args: StartTestServerArgs): Promise<StartTestServerResult> {
    ...
    const { server } = await new Promise<{ server: ReturnType<typeof createHTTPServer> }>(
        (resolve) => {
            ...
            const newServer = createHTTPServer(async (req, res) => {
                ...
                if (
                    req.url?.startsWith("/echo/") ||
                    ["/get", "/post", "/put"].includes(req.url?.split("?")[0] || "")
                ) {
                    ...
                    res.writeHead(200, {
                        ...
                        "access-control-allow-origin": "*",
                        "access-control-expose-headers":
                            "x-wasmtime-test-method, x-wasmtime-test-uri",
                    });
                    res.end(body);
                    return;
                }
                ...
                try {
                    ...
                    res.writeHead(200, {
                        "content-type":
                            mime.getType(extname(req.url || "")) || "application/octet-stream",
                    });
                    res.end(html);
                } catch (e: any) {
                    if (e.code === "ENOENT") {
                        if (debug) {
                            console.error(
                                `[server] ERROR: no such file [${req.url}] (fileURL ? [${fileURL}]): ${e}`,
                            );
                        }
                        res.writeHead(404);
                        res.end(e.message);
                        ...
                        if (debug) {
                            console.error(
                                `[server] ERROR: failed to serve URL [${req.url}] (fileURL ? [${fileURL}]): ${e}`,
                            );
                        }
                        res.writeHead(500);
                        res.end(e.message);
    ...
    const { family, address: addr, port } = address;
    const baseURL = `http://${family === "IPv6" ? "[" + addr + "]" : addr}:${port}`;
...
interface RunBasicHarnessPageTestArgs {
    browser: Browser;
    url: string;
    debug?: boolean;
}
...
/**
 * Run the test for a basic harness, by loading a given URL
 * and evaluating the body to watch completion
 */
export async function runBasicHarnessPageTest(
    args: RunBasicHarnessPageTestArgs,
): Promise<{ statusJSON: StatusJSON }> {
    const { browser, url, debug } = args;
    ...
    let statusJSON: StatusJSON = { status: "init" };
...
export function tsCodegen(args: TsCodegenArgs) {
    if (!args) {
