...
class OutgoingBody implements TypesNamespace.OutgoingBody {
  ...
  #outputStreamId: number | null = null;
  ...
  static finish(body: OutgoingBody, trailers: Fields | undefined) {
    if (trailers) {
  ...
  get() {
    ...
    if (ret.tag === "ok" && ret.val.tag === "ok") {
      ...
      ret.val.val = incomingResponseCreate(
        status,
        fieldsFromEntriesChecked(headers.map(([key, val]) => [key, textEncoder.encode(val)])),
        bodyStreamId,
      );
    }
    return ret;
