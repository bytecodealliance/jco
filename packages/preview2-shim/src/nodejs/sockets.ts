...
import { IpAddressFamily } from "../../types/interfaces/wasi-sockets-network.js";
...
class ResolveAddressStream implements IpNameLookupNamespace.ResolveAddressStream {
  #id = 0;
  #data: Array<IpNameLookupNamespace.IpAddress> | null = null;
  #curItem = 0;
  #error = false;
  #finalizer;
  resolveNextAddress() {
  ...
  static _resolveAddresses(network: NetworkNamespace.Network, name: string) {
...
function resolveAddressStreamDispose(id: number) {
...
class TcpSocket implements TcpNamespace.TcpSocket {
  ...
  static _create(addressFamily: IpAddressFamily, id: number) {
  ...
  finishConnect(): [TcpNamespace.InputStream, TcpNamespace.OutputStream] {
  ...
  accept(): [TcpNamespace.TcpSocket, TcpNamespace.InputStream, TcpNamespace.OutputStream] {
    if (!mayTcp(this.#network)) {
      throw "access-denied";
    }
  ...
  receiveBufferSize() {
    ...
    return this.#options.receiveBufferSize as unknown as bigint;
  }
  ...
  sendBufferSize() {
    ...
    return this.#options.sendBufferSize as unknown as bigint;
  }
...
class UdpSocket implements UdpNamespace.UdpSocket {
  ...
  stream(
    remoteAddress,
  ): [UdpNamespace.IncomingDatagramStream, UdpNamespace.OutgoingDatagramStream] {
