...
on Windows which changes those semantics meaningfully.

While not completely a breaking change, users that may
depend on the previous functionality should check before
updating.
...
- _(p2-shim)_ support symlink for the browser in-memory filesystem by @eduardomourar

- _(p2-shim)_ add missing socket error-code mapping by @eduardomourar in #2055
...
- _(p2-shim)_ preserve symlinks during browser path resolution by @vados-cosmonic

- _(p2-shim)_ handle Windows release conformance failures by @vados-cosmonic
...
- _(p2-shim)_ exercise browser symlinks through a component by @vados-cosmonic in #2057
...
- _(p2-shim)_ downstream worker bundling by @vados-cosmonic
...
- _(p2-shim)_ run build in CI to enable browser tests by @vados-cosmonic
