import { InputStream } from './io/streams.js';
...
class Descriptor {
  ...
  readViaStream(offset) {
    let buf = getSource(this.#entry).subarray(Number(offset));
      ...
      new ReadableStream({
        pull: (controller) => {
          if (buf.byteLength === 0) {
            ...
            controller.enqueue(buf.slice(0, 4096)); // max 4KB; slice() to copy
            buf = buf.subarray(4096);
          }
        },
        cancel: () => {
          buf = null;
