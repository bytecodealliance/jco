...
class Descriptor {
  ...
  async setTimes(atimeDesc, mtimeDesc) {
    ...
    const { atime, mtime } = await this.#computeTimestamps(atimeDesc, mtimeDesc);
  ...
  readDirectory() {
    ...
    const preopens = preopenEntries.map(([desc, _virtualPath]) => desc.#hostPreopen);
  ...
  async openAt(pf, path, of, df) {
    ...
    const makeFsFlags = () => {
      let fsFlags = 0;
      if (of.create) {
        fsFlags |= fs.constants.O_CREAT;
      }
      if (of.directory) {
        fsFlags |= fs.constants.O_DIRECTORY;
      }
      if (of.exclusive) {
        fsFlags |= fs.constants.O_EXCL;
      }
      if (of.truncate) {
      ...
      if (df.dataIntegritySync) {
        fsFlags |= fs.constants.O_DSYNC;
      }
      if (!pf.symlinkFollow) {
        fsFlags |= fs.constants.O_NOFOLLOW;
      }
      return fsFlags;
    };
  ...
  async symlinkAt(target, path) {
    ...
    try {
      ...
      if (process.platform === "win32" && ["EPERM", "EEXIST"].includes(e.code)) {
        throw new FSError("no-entry");
      }
  ...
  async #syncDir() {
    try {
      const handle = await fs.open(this.#hostPreopen, fs.constants.O_RDONLY);
      try {
        await handle.sync();
      } finally {
        await handle.close();
  ...
  #getNewTimestamp(newTimestamp, oldTimestamp) {
    switch (newTimestamp.tag) {
      ...
      default:
        throw new FSError("invalid", `Unknown new-timestamp tag: ${newTimestamp.tag}`);
    }
  }
  ...
  async #computeTimestamps(atimeDesc, mtimeDesc, path = null) {
    ...
    const atime = this.#getNewTimestamp(atimeDesc, stats?.dataAccessTimestamp);
    const mtime = this.#getNewTimestamp(mtimeDesc, stats?.dataModificationTimestamp);
    return { atime, mtime };
  }
