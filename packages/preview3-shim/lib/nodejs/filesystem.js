...
class Descriptor {
    ...
    async appendViaStream(data) {
        ...
        try {
            await _worker.runOp({ op: 'append', fd: this.#fd, stream }, [
                stream,
            ]);
        } catch (err) {
            throw mapError(err);
    ...
    async setSize(size) {
        ...
        try {
            await fs.ftruncate(this.#fd, Number(size));
        } catch (e) {
    ...
    async openAt(pf, path, of, df) {
        ...
        if (df.requestedWriteSync || df.mutateDirectory) throw 'unsupported';
        if (df.fileIntegritySync || df.dataIntegritySync) throw 'unsupported';
        ...
        try {
            ...
            const desc = descriptorCreate(handle.fd, df, fullPath);
