...
import { readComponentBytes, nodeExec, setupAsyncTest } from './helpers.js';
...
suite('memory', () => {
    ...
    test.concurrent('does not leak during numerous transpile calls', async () => {
        ...
        const { files } = await transpileBytes(adderComponentBytes, {
            outDir,
        });
        await Promise.all(
...
process.stdout.write(JSON.stringify({ runs, memoryUsage: { before, after } }));
        ...
        const { stdout } = await nodeExec(testJsOutputPath);
