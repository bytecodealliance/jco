...
suite(`TypeScript`, async () => {
    test.concurrent(`TS aliasing`, async () => {
        ...
        const { files } = await transpileBytes(component, { name: 'issue' });
    ...
    test.concurrent(`TS types`, async () => {
        const witSource = await readFile(
            fileURLToPath(new URL(`./fixtures/wit/issue-480/issue-480.wit`, import.meta.url)),
            'utf8',
        );
        const component = await componentNew(
            await componentEmbed({
    ...
    test.concurrent(`Disposable interface generation (host-types, import) `, async () => {
        ...
        const { files } = await transpileBytes(component, { name: 'disposable' });
    ...
    test.concurrent(`Disposable interface generation (guest-types, import) `, async () => {
        const witPath = fileURLToPath(
            new URL(`./fixtures/wit/disposable-resources/disposable-resources.wit`, import.meta.url),
        );
    ...
    test.concurrent(`helper type aliases stay at top level in instantiation mode (issue 1708)`, async () => {
        const witSource = await readFile(
            fileURLToPath(new URL(`./fixtures/wit/issue-1708/issue-1708.wit`, import.meta.url)),
            'utf8',
        );
        const component = await componentNew(
            await componentEmbed({
        ...
        // interface body.
        const interfaceOpenPos = dtsSource.indexOf(`export interface Root {`);
        const interfaceClosePos = dtsSource.indexOf(`}`, interfaceOpenPos);
        const resultAliasPos = dtsSource.indexOf(`export type Result<T, E>`);
        const optionAliasPos = dtsSource.indexOf(`export type Option<T>`);
        assert.ok(interfaceOpenPos !== -1, 'world interface should be generated in instantiation mode');
        assert.ok(
            resultAliasPos > interfaceClosePos,
            'Result helper type alias must be emitted outside the world interface',
        );
        assert.ok(
            optionAliasPos > interfaceClosePos,
            'Option helper type alias must be emitted outside the world interface',
        );
    });
    ...
    test.concurrent(`sync lowered async fn returns Promse<T> (guest-types)`, async () => {
        const witPath = fileURLToPath(new URL(`./fixtures/wit/async-flat-param-adder/test.wit`, import.meta.url));
        const files = await generateGuestTypes(witPath);
