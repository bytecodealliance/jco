...
test("clearMarks clears only matching type and name", () => {
  p.mark("", { startTime: 0 });
  ...
  p.clearMarks("");
  expect(p.getEntriesByName("").map((e) => e.entryType)).not.toContain("mark");
  p.clearMarks();
  expect(p.getEntriesByType("mark")).toEqual([]);
});
