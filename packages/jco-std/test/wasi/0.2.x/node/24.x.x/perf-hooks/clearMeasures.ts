...
test("clearMeasures clears only matching type and name", () => {
  p.mark("", { startTime: 0 });
  ...
  p.clearMeasures("");
  expect(p.getEntriesByName("").map((e) => e.entryType)).not.toContain("measure");
  p.clearMeasures();
  expect(p.getEntriesByType("measure")).toEqual([]);
});
