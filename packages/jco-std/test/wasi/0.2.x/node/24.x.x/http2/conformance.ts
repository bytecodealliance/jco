...
function directHarness() {
  ...
  let id: number;
  const implementation = createDirectHttp2Implementation({
    ClientSession: class {
      ready = () => ({
        tag: "ok" as const,
        val: {
    ...
    Server: class {
      constructor(_options: unknown, value: number, _errorListener: number) {
        id = value;
      }
      listen = () => ({
        tag: "ok" as const,
  ...
  return {
    ...
    async dispatch(stream: Http2IncomingStreamData) {
      listener ??= implementation.http2Callbacks.takeStreamListener(id);
