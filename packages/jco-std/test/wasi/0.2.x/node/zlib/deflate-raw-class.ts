...
test("DeflateRaw streams multiple writes with native state", async () => {
  const stream = zlib.DeflateRaw();
  expect(stream).toBeInstanceOf(zlib.DeflateRaw);
  const output = await collect(stream, [input.subarray(0, 5), input.subarray(5)]);
  expect(output).toEqual(native.deflateRawSync(input));
