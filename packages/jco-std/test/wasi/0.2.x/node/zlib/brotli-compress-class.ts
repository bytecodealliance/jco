...
test("BrotliCompress streams multiple writes with native state", async () => {
  const stream = zlib.BrotliCompress();
  expect(stream).toBeInstanceOf(zlib.BrotliCompress);
  const output = await collect(stream, [input.subarray(0, 5), input.subarray(5)]);
  expect(output).toEqual(native.brotliCompressSync(input));
