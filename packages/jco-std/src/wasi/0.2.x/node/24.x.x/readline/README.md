...
imports, process objects, or unrelated WASI capabilities are required.
...
- Deferred callbacks and auto-commit use the engine's microtask queue, not Node's
  ...
  can differ.
- Terminal mode uses ANSI sequences. Ambient `TERM=dumb` detection is unavailable.
...
- Ctrl+Z emits `SIGTSTP` when handled; otherwise it throws
  `ERR_JCO_UNSUPPORTED_NODE_API`. Components cannot suspend a host process or
  receive its job-control `SIGCONT` events.
