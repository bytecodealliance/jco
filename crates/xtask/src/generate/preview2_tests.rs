...
let test_run = || {{
    ...
    if !{should_error}status.success() {{ anyhow::bail!("executable returned non-zero exit code, retrying..."); }}
...
"#
    ...
    //     // Add in checks for whether test execution failed, if necessary
    //     src.push_str(&format!(
    //         r#"
